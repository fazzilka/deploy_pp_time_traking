...
async def start_timer(session: AsyncSession, task_id: int) -> Task:
    async with session.begin():
        ...
        if active_interval is not None:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Активный интервал уже существует")
...
async def stop_timer(session: AsyncSession, task_id: int) -> Task:
    async with session.begin():
        ...
        if active_interval is None:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Активный интервал не найден")
        finished_at = utc_now()
